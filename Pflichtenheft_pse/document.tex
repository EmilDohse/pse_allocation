...
und eine Bewertung von \gls{Einteilung}en vorgenommen werden
}}
...
plural=Systeme, description={Das in Betrieb befindliche Produkt}} 
...
\newglossaryentry{Voraussetzung}{name=Voraussetzung, plural=Voraussetzungen,
	description={Eine \gls{Voraussetzung} ist eine formale Bedingungen, die laut
	Prüfungsordnung erfüllt sein müssen, damit ein \gls{Studierender} beim
	\gls{PSE} teilnehmen darf}}
...
genau eine Festlegung dieser Parameter und Kriterien auf einen konkreten Wert}}
	...
	\newglossaryentry{Studierendenbereich}{name=Studierendenbereich,
plural=Studierendenbereiche, description={Der Bereich der Webseite der Benutzern
die als Studierende angemeldet sind zur Verfügung steht}}

	\newglossaryentry{Betreuerbereich}{name=Betreuerbereich,
plural=Betreuerbereiche, description={Der Bereich der Webseite der Benutzern
die als Betreuer angemeldet sind zur Verfügung steht}}

	\newglossaryentry{Administratorbereich}{name=Administratorbereich,
plural=Administratorbereiche, description={Der Bereich der Webseite der Benutzern
die als Administrator angemeldet sind zur Verfügung steht}}
...
\newglossaryentry{HTTP}{name=HTTP,
	description={Das Hypertext Transfer Protocol ist ein Protokoll zur Übertragung von Daten über ein Rechnernetz. Es wird hauptsächlich eingesetzt, um Webseiten (Hypertext-Dokumente) aus dem World Wide Web (WWW) in einen Webbrowser zu laden. Es ist jedoch nicht prinzipiell darauf beschränkt}}
	
\newglossaryentry{SMTP}{name=SMTP,
	description={Das Simple Mail Transfer Protocol ist ein Protokoll der Internetprotokollfamilie, das zum Austausch von E-Mails in Computernetzen dient. Es wird dabei vorrangig zum Einspeisen und zum Weiterleiten von E-Mails verwendet. Zum Abholen von Nachrichten kommen andere, spezialisierte Protokolle zum Einsatz. SMTP-Server nehmen traditionell Verbindungen auf Port 25 („smtp“) entgegen}}
...
Dadurch ist die aktuelle Lösung mit \enquote{Webinscribe} umständlich in der Bedienung und lässt Wünsche bezüglich hilfreicher Features offen.
...
\begin{itemize} 
  \item Webserver (\texttt{\gls{HTTP}, \gls{SMTP}})
  \item SQL"=Server
...
\begin{enumerate}[label=\swtLabel{FA}, resume]
  ...
  \testRef{UCbetreuerThemaErstellen}
  \item Ändern der \gls{Projektdetails}: Name, Beschreibung, \gls{Projektbetreuer},
        minimale und maximale Teilnehmerzahl, Anzahl der \glspl{Team}
        \label{FAbetreuerChangeProjekt} \testRef{UCbetreuerThemaÄndern}
...
\begin{enumerate}[label=\swtLabel{D}, resume] 
  \item Name
  \item Passwort
  \item \gls{Bewertung} der \glspl{Projekt}, zu welchen die \glspl{Lerngruppe}mitglieder
  (\glspl{Studierender}) am liebsten zugeteilt würden
...
\begin{enumerate}[label=\swtLabel{NF}]
  ...
  Stunden fertig
	...
	Minuten
	\item Benutzbarkeit (\gls{Admin}): Nach Einführung in die Benutzung des
	Adminbereichs von 4h können Nutzer die \gls{Einteilung} administrieren
...
Der Test der funktionalen Anforderungen verläuft primär über die
Anwendungsfälle, welche über Verweise nach jeder funktionalen Anforderung direkt
erreichbar sind. 

Folgende Funktionssequenzen sind noch zusätzlich zu prüfen:
...
\begin{enumerate} [label=\swtLabel{T}]
  ...
  \item Fristen zur Registrierung als \gls{Studierender} und zur
  \gls{Bewertung}sabgabe werden eingehalten (es werden immer die des aktiven
  Semesters verwendet) 
...
\begin{enumerate} [label=\swtLabel{A}, itemsep=2em]
	...
	\begin{description}
		...
		\item[Alternativen:]
		\begin{enumerate}
			\item[2a)] Kann entfallen, wenn Server bereits in Betrieb sind
		\end{enumerate}
    ...
    \item[Ziel:] \gls{Admin} soll angemeldet werden
  	\item[Vorbedingung:] -keine-
  	...
  	\item[Beschreibung:]
  	\begin{enumerate}
  	  \item[1.] \gls{Admin} öffnet die Webseite (\autoref{GUIindex})
  	  \item[2.] \gls{Admin} füllt das Formular mit \gls{Benutzername} und
  	  Passwort aus und klickt auf \enquote{Login}
  ...
  \end{description}
	...
	\begin{description}
		...
		\item[Ziel:] Ein frühestmöglicher Zeitpunkt für die Registrierung und
		Anmeldung von \glspl{Studierender}n soll festgelegt werden
	...
	\begin{description}
		...
		\item[Ziel:] Ein spätester Zeitpunkt für die Anmeldung von
		\glspl{Studierender}n soll festgelegt werden
		\item[Vorbedingung:] \gls{Admin} ist angemeldet und im \gls{Admin}bereich der Webseite
  ...
  \item \label{UCadminProjektErstellenÄndern}
    \begin{description}
  	...
  	\item[Beschreibung:]
  	\begin{enumerate} 
  	  \item[1.] \gls{Projekt}übersicht über einen klick auf \enquote{Projekte}
  	  öffnen (\autoref{GUIprojUebersicht})
  	  \item[2.] Wenn das \gls{Projekt} schon existiert, so wird es über einen
  	  klick auf den \gls{Projekt}namen geöffnet 
  	  
  	  \item[3.] Nun können Änderungen der Projektdaten in der Eingabemaske
  	  getätigt werden (\autoref{GUIprojEdit})
  	  \item[5.] Durch Abspeichern der Eingaben über den \enquote{Speichern}-Knopf
  	  werden die Daten in der Datenbank geändert/hinzugefügt
  	\end{enumerate}
  	\item[Erweiterungen:] -keine-
  	\item[Alternativen:]
  	\begin{enumerate}
  	  \item[2a)] Wenn nicht, dann wird das \gls{Projekt} über einen klick auf \enquote{neues Projekt} erstellt
  	  \item[3a)] Falls es sich um ein neues Projekt handelt müssen alle Daten neu
  	  eingegeben werden
  	\end{enumerate} 
  ...
  \item[Ziel:] Löschen eines \glspl{Projekt}s aus der Datenbank
  	\item[Vorbedingung:] \gls{Admin} ist angemeldet und im \gls{Admin}bereich der
  	Webseite. Weiterhin existiert das \gls{Projekt} bereits in der Datenbank und die
  	...
  	\item[Beschreibung:]
  	\begin{enumerate} 
  	  \item[1.] Über einen klick auf \enquote{Projekte} die
  	  \gls{Projekt}übersicht öffnen (\autoref{GUIprojUebersicht})
  	  \item[2.] Klick auf den Namen des zu löschenden \gls{Projekt}s aus der
  	  Liste
  	  \item[3.] Nun ist die Projekteingabe geöffnet (\autoref{GUIprojEdit})
  	  \item[4.] Klick auf \enquote{Projekt löschen}
  	  \item[5.] Ein Warnhinweis erscheint welcher mit \enquote{Ok} zu bestätigen
  	  ist
  	  \item[6.] Das \gls{Projekt} ist nun gelöscht
  	\end{enumerate}
  	\item[Erweiterungen:] -keine-
  	\item[Alternativen:] -keine-
  ...
  \item[Ziel:] Finden einer \gls{Einteilung} für eine bestimmte \gls{Einteilungskonfiguration}
  	...
  	und der \gls{Admin} befindet sich im Reiter \enquote{Einteilung}
  	...
  	\item[Beschreibung:]
  	\begin{enumerate} 
  	  \item[1.] Einstellen der \gls{Einteilungskonfiguration}, nach der eingeteilt werden
  	  soll (siehe \autoref{GUIeinteilung})
  	  \item[2.] Durch Klick auf \enquote{Zur Berechnung hinzufügen} wird die
  	  Berechnung mit dieser \gls{Einteilungskonfiguration} gestartet
  	...
  	\item[Alternativen:] 
  	\begin{enumerate}
  	  \item[3a)] Hier ist auch ein frühzeitiger Abbruch durch einen klick auf
  	  \enquote{Ausgewählte Einteilung stoppen} möglich.
  	  Dies wird im Ergebnis vermerkt
  	 \end{enumerate}  
  ...
  \item[Ziel:] Finden der bestmöglichen \gls{Einteilung}
  	\item[Vorbedingung:] Es wurde mindestens eine \gls{Einteilung} berechnet und
  	der \gls{Admin} befindet sich im Administratorbereich
  	...
  	\item[Beschreibung:]
  	\begin{enumerate} 
  	  \item[1.] Öffnen der \gls{Einteilung}sübersicht über klick auf 
  	  \enquote{Einteilungsergebnisse} (\autoref{GUIeinteilungErgeb})
  	...
  	\item[Erweiterungen:]
  	\begin{enumerate}
  	  \item[4.] \gls{Projektbetreuer} und \glspl{Studierender} werden per E"=Mail
  	  über die \gls{Einteilung} informiert
  	 \end{enumerate}
  	\item[Alternativen:] -keine-
  ...
  \item[Ziel:] Hinzufügen eines \gls{Projektbetreuer}s in das Produkt
  	\item[Vorbedingung:] Der \gls{Admin} befindet sich im Administratorbereich
  	...
  	\item[Beschreibung:]
  	 \begin{enumerate} 
  	   \item[1.] Öffnen der Betreuerübersicht über einen Klick auf
  	   \enquote{Projektleiter} (\autoref{GUIprojLeiterUebersicht})
  	   \item[2.] Ausfüllen der Betreuerdaten über dem Knopf \enquote{Betreuer
  	   hinzufügen}
  	   \item[3.] Durch Klick auf \enquote{Projektleiter hinzufügen} wird der neu
  	   angelegte \gls{Projektbetreuer} dem Produkt hinzugefügt
  	 \end{enumerate} 
  	\item[Erweiterungen:]
  	 \begin{enumerate}
  ...
  \item \label{UCadminEinteilungUebersicht}
    \begin{description}
      ...
      (\autoref{GUIeinteilungErgeb})
      \item[3.] Der \gls{Admin} wählt einen \gls{Einteilung}sreiter aus
  ...
  \item \label{UCadminSPOteilleistungAdd}
    ...
    \glspl{Teilleistung} zur Auswahl angeboten
    \item[Vorbedingung:] Der \gls{Admin} ist angemeldet und befindet sich im \gls{Admin}bereich
    \item[Nachbedingung:]	\begin{description}[leftmargin=6em] \item[Erfolg]  Der \gls{Admin} ist im Bereich
    \enquote{Einstellungen} und hat eine neue \gls{Teilleistung} zu einer \gls{SPO}
    hinzugefügt
    \item[Fehlschlag] Es wurde keine \gls{Teilleistung} zu einer \gls{SPO} 
    hinzugefügt \end{description}
    \item[Beschreibung:]
    \begin{enumerate}
      \item[1.] \gls{Admin} ist im \gls{Admin}bereich der Webseite
      \item[2.] Klick auf \enquote{Einstellungen} in der Seitenleiste
      (\autoref{GUIeinstellungen})
      \item[3.] Der \gls{Admin} wählt unter \enquote{SPOs} die \gls{SPO}, der er eine
      \gls{Teilleistung} hinzufügen möchte
      \item[4.] Der \gls{Admin} gibt unter \enquote{SPOs} in das Textfeld
      neben \enquote{Teilleistungen hinzufügen} den Namen der neuen
      \gls{Teilleistung} ein
      \item[5.] Der \gls{Admin} klickt auf \enquote{Teilleistung hinzufügen}
      \item[6.] Die \gls{Teilleistung} erscheint nun in der Aufzählung der
  ...
  \item \label{UCadminSPOwelcheWerdeAngezeigt}
    ...
    \glspl{Studierender}n angezeigt
    \item[Vorbedingung:] Der \gls{Admin} ist angemeldet und befindet sich im \gls{Admin}bereich
    \item[Nachbedingung:]	\begin{description}[leftmargin=6em] \item[Erfolg]  Der \gls{Admin} ist im Bereich
    \enquote{Einstellungen} und nur noch die relevanten \glspl{SPO} werden für
  ...
  \item \label{UCadminSPOteilleistungRM}
    ...
    \gls{Teilleistung} nicht mehr angezeigt
    ...
    \gls{Teilleistung}
    \item[Nachbedingung:]	\begin{description}[leftmargin=6em] \item[Erfolg]  Der \gls{Admin} ist im Bereich
    \enquote{Einstellungen} und die entfernte \gls{Teilleistung} wird in der \gls{SPO}
    nicht mehr angezeigt
    \item[Fehlschlag] Es wurde keine \gls{Teilleistung} entfernt \end{description}
    \item[Beschreibung:]
    \begin{enumerate}
      \item[1.] \gls{Admin} ist im \gls{Admin}bereich der Webseite
      \item[2.] Klick auf \enquote{Einstellungen} in der Seitenleiste
      (\autoref{GUIeinstellungen})
       ...
       eine \gls{Teilleistung} entfernen möchte
      \item[4.] Der \gls{Admin} setzt die Checkbox bei den \glspl{Teilleistung} die er
      entfernen möchte
      \item[5.] Der \gls{Admin} klickt auf \enquote{markierte
      \glspl{Teilleistung} löschen}
      \item[5.] Die \gls{Teilleistung} erscheint nun nicht mehr in der Aufzählung der
      \glspl{Teilleistung} 
    \end{enumerate}
    \item[Erweiterungen:] -keine-
    \item[Alternativen:] -keine-
  ...
  \end{description}
    ...
    \begin{enumerate}
      ...
      in der Seitenleiste (\autoref{GUIexportimport})
  ...
  \end{description}
    ...
    \begin{enumerate}
      \item[1.] Der \gls{Admin} ist im \gls{Admin}bereich der Webseite
      \item[2.] Klick auf \enquote{Export/Import}
      in der Seitenleiste (\autoref{GUIeinstellungen})
  ...
  \end{description}
    ...
    \item[Fehlschlag] Der \gls{Admin} erhält keine Auskunft über Konflikte 
    \end{description}
    \item[Beschreibung:]
    \begin{enumerate}
      \item[1.] Der \gls{Admin} ist im \gls{Admin}bereich der Webseite
  ...
  \item \label{UCadminVergleichGuete}
    \begin{description}
  	...
  	\begin{enumerate}
  	  \item[1.] \gls{Admin} ist im \gls{Admin}bereich der Webseite
      \item[2.] Klick auf \enquote{\gls{Einteilung}sergebnisse} in der
      Seitenleiste (\autoref{GUIeinteilungErgeb})
      \item[3.] Der \gls{Admin} wählt den Reiter \enquote{Vergleich} aus
      (\autoref{GUIvergleich})
  ...
  \item \label{UCadminStapelverarbeitung}
    \begin{description}
  	...
  	\item[Vorbedingung:] Der \gls{Admin} befindet sich im Administratorbereich.
  	Bewertungsdeadline der Studierenden ist abgelaufen
  ...
  \item \label{UCadminCreateStudierender}
    \begin{description}
  	...
  	\begin{enumerate}
  	  ...
  	  hinzufügen/löschen} (\autoref{GUIstudentEdit})
  ...
  \item \label{UCadminAddStudToTeam}
    \begin{description}
  	...
  	\item[Ziel:] Einen nicht zugeteilten \glspl{Studierender}n manuell zu einem
  	\gls{Team} hinzufügen
  	\item[Vorbedingung:] Der \gls{Admin} befindet sich im Bereich
  	\enquote{\gls{Einteilung}sergebnisse} 
  	...
  	\item[Erweiterungen:]
  	\begin{enumerate}
  	  \item [4a)] Überschreitet der \gls{Admin} die von ihm eingestellten
  ...
  \item \label{UCadminRemoveStudFromTeam}
    \begin{description}
  	...
  	\item[Vorbedingung:] Der \gls{Admin} befindet sich im Bereich
  	\enquote{\gls{Einteilung}sergebnisse}
  	\item[Nachbedingung:]	\begin{description}[leftmargin=6em] \item[Erfolg]  Der
  	\glspl{Studierender} wurde aus dem \gls{Team} entfernt und befindet sich
  	bei \enquote{Nicht zugeteilte Personen}
  ...
  \item \label{UCadminSwapStudsBetweenTeams}
    \begin{description}
      ...
      \item[3.] Klick auf \enquote{markierte 2 Personen tauschen}
  	\end{enumerate}
  	\item[Erweiterungen:]
  	\begin{enumerate}
  	  \item [4a)] Überschreitet der \gls{Admin} die von ihm eingestellten
...
\begin{enumerate}[label=\swtLabel{S}, itemsep=2em]
    ...
    \begin{description}
  	...
  	\item[Vorbedingung:] Registrierung wurde freigeschaltet (d.h. der Zeitpunkt
  	der ausführung liegt zwischen der \enquote{Startzeit der
  	Studierendenanmeldung} und der \enquote{Deadline für Projektbewertungen})
  	...
  	\item[Beschreibung:]
  	\begin{enumerate}
  	  \item[1.] Öffnen der Internetseite (\autoref{GUIindex})
      \item[2.] Klick auf \enquote{Registrierung} (\autoref{GUIregi})
      \item[3.] Ausfüllen der Registrierungsmaske 
      \item[4.] Abschicken der Daten über einen klick auf \enquote{Registrieren}
      \item[5.] Existiert kein Account mit dieser Matrikelnummer, so wird der \glspl{Studierender} der Datenbank hinzugefügt
  	\end{enumerate}
  	\item[Erweiterungen:]
  	\begin{enumerate}
  	  \item[3a)] Statt der Registrierungsmaske wird hier der \gls{SCC-Account}
  	  verwendet
  	  \item[4a)] Dem \glspl{Studierender}n wird nach dem Abschicken seiner Daten
  	  eine Verifikations-Email gesendet. (siehe
  	  \testRef{UCstudVerifikationEmail})
  	 \end{enumerate} 
  	\item[Alternativen:]
  	\begin{enumerate}
...
%   
  ...
  \item \label{UCstudVerifikationEmail}
    ...
    \item[Vorbedingung:] \gls{Studierender} muss sich vor unter 24h registriert
    haben und seine E"=Mail"=Adresse korrekt angegeben haben.
    \item[Nachbedingung:]	\begin{description}[leftmargin=6em] \item[Erfolg] 
    \gls{Studierender} kann sich nun anmelden und hat Zugriff auf den \glspl{Studierender}nbereich
    ...
    \begin{enumerate}
      ...
      \item[2.] Klick auf den Verifikations-Link in der Verifikations"=E"=Mail.    
      \item[3.] Bei erfolgreicher Verrifikation findet eine Weiterleitung auf
      die Startseite des Produkts findet statt (\autoref{GUIindex})
    \end{enumerate}
    \item[Erweiterungen:] -keine-
    \item[Alternativen:] \begin{enumerate}    
      \item[3a)] Falls die Verrifikation nicht erfolgreich ist und keine
      Weiterleitung auf die Startseite erfolgt Schritt 2 wiederholen
    \end{enumerate}
  ...
  \item \label{UCstudAnmeldung}
    ...
    \item[Ziel:] \gls{Studierender} wird angemeldet befindet sich im
    Studierendenbereich
  	\item[Vorbedingung:] Der \glspl{Studierender} hat sich registriert
  	\item[Nachbedingung:]	\begin{description}[leftmargin=6em] \item[Erfolg]  Der
  	\glspl{Studierender} ist angemeldet und befindet sich im Studierendenbereich
  	im Reiter \enquote{Lerngruppe}
  	\item[Fehlschlag] Der \glspl{Studierender} ist nicht angemeldet \end{description}
  	\item[Beschreibung:]
  	\begin{enumerate}
  	  \item[1.] \gls{Studierender} öffnet die Startseite des Produkts
  	  (\autoref{GUIindex})
  	  \item[2.] \gls{Studierender} füllt das Formular zum Anmelden aus und schickt
  	  dieses über einen Klick auf \enquote{Login} ab 
  	  \item[3.] Sind die Anmeldedaten korrekt, so wird der \glspl{Studierender} zum
  	  \glspl{Studierender}nbereich weitergeleitet
  	\end{enumerate}
  	\item[Erweiterungen:]
  	\begin{enumerate}
  	  \item[2a)] Kein Formular, sondern Anmeldung über \gls{SCC-Account}
  	\end{enumerate}    	
  	\item[Alternativen:]
	\begin{enumerate}
  	  \item[3a)] Wenn nicht, so erhält er eine Fehlermeldung
  ...
  Ersteller ist erstes Mitglied dieser
  	\item[Vorbedingung:] \gls{Studierender} ist angemeldet und befindet im
  	Studierendenbereich
  	...
  	\item[Beschreibung:]
  	\begin{enumerate}
  	  \item[1.] Klick auf \enquote{Lerngruppe} (\autoref{GUIanmeldLG})
  	  \item[2.] \gls{Studierender} gibt Name und Passwort in das Formular zum Erstellen
  	  einer \gls{Lerngruppe} ein und schickt dieses über einen Klick auf
  	  \enquote{Lerngruppe erstellen} ab
  	  
  	  \item[3.] Wenn noch keine \gls{Lerngruppe} mit diesem Namen existiert, so
  ...
  \item[Ziel:] \gls{Studierender} ist Mitglied einer \gls{Lerngruppe}
  	...
  	\gls{Lerngruppe} und teilt somit die Projektbewertungen der Lerngruppe
  	\item[Fehlschlag] Der \glspl{Studierender} ist nicht Mitglied der 
  	\gls{Lerngruppe} \end{description}
  	\item[Beschreibung:]
  	\begin{enumerate}
  	  \item[1.] Klick auf \enquote{Lerngruppe} (\autoref{GUIanmeldLG})
  	  \item[2.] Der \glspl{Studierender} gibt die Anmeldedaten der
  	  \gls{Lerngruppe} in das passende Formular ein und schickt dieses ab 
  	  \item[3.] Existiert die angegebene \gls{Lerngruppe}, so wird die
  	...
  	\begin{enumerate}
  	  ...
  	  der \glspl{Studierender} ist kein Mitglied einer \gls{Lerngruppe}
  	 \end{enumerate}
  ...
  \item[Ziel:] \gls{Studierender} kann sehen wer in seiner \gls{Lerngruppe} ist
  	\item[Vorbedingung:] \gls{Studierender} ist angemeldet und befindet sich im \glspl{Studierender}nbereich. Weiterhin ist der \glspl{Studierender} Mitglied einer
  	\gls{Lerngruppe}
  ...
  zugeteilt wurde
  	\item[Vorbedingung:] \gls{Studierender} ist angemeldet und befindet sich im
  	\glspl{Studierender}nbereich
  ...
  \item[Ziel:] \gls{Studierender} kann sich während der Projektbewertung die
  Beschreibung der zu bewertenden \glspl{Projekt} nochmals ansehen
  	\item[Vorbedingung:] \gls{Studierender} ist angemeldet und im
  	\gls{Studierendenbereich}
  	...
  	\item[Beschreibung:]
  	\begin{enumerate}
  	  \item [1.] Klick auf \enquote{Projektbewertung}
  	  (\autoref{GUIbewertung})
  	  \item[2.] Der \glspl{Studierender} fährt mit seiner Maus über den \gls{Projekt}namen
  ...
  \item[Ziel:] \gls{Studierender} tritt aus einer \gls{Lerngruppe} aus
  	\item[Vorbedingung:] \gls{Studierender} ist angemeldet und befindet sich im
  	\glspl{Studierender}nbereich. Weiterhin ist er Mitglied einer \gls{Lerngruppe}
  	...
  	\item[Beschreibung:]
  	\begin{enumerate}
  	  \item[1.] Der \glspl{Studierender} klickt auf \enquote{\gls{Lerngruppe}}.
  	  Nun befindet er sich in der \enquote{\gls{Lerngruppe}}-Maske (\autoref{GUIlerngUebersicht})
  	  \item[2.] Der \glspl{Studierender} klickt auf \enquote{Austreten}
 
  	\end{enumerate}
  	\item[Erweiterungen:] 
		\begin{enumerate}
		  \item Die zuletzt gespeicherten Projektbewertungen des \gls{Studierender} werden
		  verwendet
		\end{enumerate}
  	\item[Alternativen:] -keine-
...
%   
  ...
  \item[Ziel:] Der \glspl{Studierender} hat eine \gls{Bewertung} abgegeben
  	\item[Vorbedingung:] Der \glspl{Studierender} ist angemeldet und im
  	\glspl{Studierender}nbereich
  	...
  	\item[Beschreibung:]
  	 \begin{enumerate}
  	   \item [1.] Klick auf \enquote{Projektbewertung} (\autoref{GUIbewertung})
  	   \item[2.] Der \glspl{Studierender} füllt die \gls{Bewertung}smaske aus
  	   
  	   \item[3.] Der \glspl{Studierender} speichert die \gls{Bewertung} über
  	   einen klick auf \enquote{Bewertung abgeben} ab
  	   \item[4.] Ist der \glspl{Studierender} Mitglied einer \gls{Lerngruppe},
  	   so gilt die \gls{Bewertung} gleichzeitig auch für alle anderen Mitglieder
  	 \end{enumerate}
  	\item[Erweiterungen:] -keine-
  	\item[Alternativen:]
  	\begin{enumerate}
  	  \item[4a)] Wenn nicht, so gilt sie nur für den \glspl{Studierender}n
  	 \end{enumerate}
  ...
  \item \label{UCstudNewPasswort}
    \begin{description}
  	...
  	\item[Ziel:] Erhalten eines neuen, zufallsgenerierten Passworts
  	...
  	\item[Alternativen:]
	\begin{enumerate}
		\item[4a)] Ist die E"=Mail"=Adresse nicht im System gespeichert, so erhält
		der \gls{Studierender} eine Fehlermeldung
	\end{enumerate}
  ...
  \item \label{UCstudLogout}
    \begin{description}
  	\item[Abmeldung]
  	\item[Ziel:] \gls{Studierender} meldet sich ab
...
\begin{enumerate} [label=\swtLabel{B}, itemsep=2em]
  \item \label{UCbetreuerAnmeldung}
	 \begin{description}
		\item[\gls{Projektbetreuer} meldet sich an]
  		...
  		\item[Beschreibung]~
  		\begin{enumerate}
  		  \item [1.] Der \gls{Projektbetreuer} befindet sich auf der Startseite des Produkts
  		  (\autoref{GUIindex})
  			\item[2.] Der \gls{Projektbetreuer} gibt seine Zugangsdaten ein
  			\item[3.] Wenn die Anmeldedaten korrekt sind, wird er auf den
  			Betreuerbereich weitergeleitet
  		\end{enumerate}
  		\item[Erweiterungen:] -keine-
  		\item[Alternativen:]
  		\begin{enumerate}
  		  \item[3a)] Wenn nicht, erhält er eine Fehlermeldung
  		\end{enumerate}  
  		\item[Benötigte FA:] \ref{FAbetreuerAnmeldung}
  	\end{description}
   
  
  \item \label{UCbetreuerThemaErstellen}
	\begin{description}
  		\item[\gls{Projekt} erstellen]
  		\item[Ziel:] Eröffnung eines \glspl{Projekt}s
  		\item[Vorbedingung:] \gls{Projektbetreuer} ist angemeldet und im
  		\gls{Betreuerbereich}. Es ist noch vor der \enquote{Startzeit der
  		Studierendenanmeldung}
  		\item[Nachbedingung:]	\begin{description}[leftmargin=6em] \item[Erfolg] 
  		\gls{Projekt}daten sind im System eingetragen. Der Betreuer ist der erste
  		Betreuer dieses Projekts. In der Seitenleise der GUI dieses Betreuers taucht
  		das Projekt auf
  		\item[Fehlschlag] \gls{Projekt}daten sind nicht im \gls{System} 
  		eingetragen \end{description}
  		\item[Beschreibung:]
  	\begin{enumerate}
  	  \item[1.] \gls{Projektbetreuer} befindet sich im Projektbetereurbereich
  	  (\autoref{GUIprojEdit})
  	  \item[3.] \gls{Projektbetreuer} füllt die Eingabemaske aus
  	  \item[4.] \gls{Projektbetreuer} speichert die neuen Daten über einen Klick
  	  auf \enquote{Speichern} ab
  	\end{enumerate}
  	\item[Erweiterungen:] -keine-
  	\item[Alternativen:] -keine- 
  	\item[Benötigte FA:] \ref{FAbetreuerAddProjekt}, 
  \end{description}
  ...
  \item \label{UCbetreuerJoinProjekt}
	\begin{description}
  		...
  		\item[Ziel:] \gls{Projektbetreuer} eines \glspl{Projekt}s werden
  		\item[Vorbedingung:] Der Benutzer ist noch nicht \gls{Projektbetreuer} des
  		ausgewählten \glspl{Projekt}s. Es ist noch vor der \enquote{Startzeit der
  		Studierendenanmeldung}
  		\item[Nachbedingung:]	\begin{description}[leftmargin=6em] \item[Erfolg]  Erfolgreich \gls{Projektbetreuer} des ausgewählten
  		\glspl{Projekt}s geworden. Das Projekt erscheint in der Seitenleiste der GUI
  		dieses Benutzers
  		\item[Fehlschlag] Der Benutzer ist nicht \gls{Projektbetreuer} des
  		\glspl{Projekt}s
  		\end{description}
  		\item[Beschreibung:]
  	\begin{enumerate} 
  	  \item[1.] \gls{Projektbetreuer} befindet sich auf der Webseite in 
  	  Beteruersicht (\autoref{GUIprojEdit})
  	  \item[2.] Klick auf \enquote{Existierendem Projekt beitreten}
  	  \item[3.] Eine Ansicht vergleichbar mit \autoref{GUIprojUebersicht} öffnet
  	  sich
  	  \item[4.] Klick auf den Namen des Projekts führt zum Beitritt
  	\end{enumerate}
  	\item[Erweiterungen:] -keine-
  	\item[Alternativen:] -keine-
  ...
  \item \label{UCbetreuerThemaÄndern}
	\begin{description}
  		\item[\gls{Projekt} ändern]
  		\item[Ziel:] Änderung eines \glspl{Projekt}s
  		\item[Vorbedingung:] \gls{Projektbetreuer} ist angemeldet und im
  		\gls{Betreuerbereich}. Es ist noch vor der \enquote{Startzeit der
  		Studierendenanmeldung}. Der Betreuer ist \gls{Projektbetreuer} des zu
  		ändernden Projekts
  		...
  		\item[Beschreibung:]
  	\begin{enumerate}
  	  \item[1.] \gls{Projektbetreuer} befindet sich im Betreuerbereich
  	  \item[2.] Klick auf das zu ändernde Projekt in der Seitenleise der GUI
  	  \item[4.] Die Projekteingabemaske öffnet sich (\autoref{GUIprojEdit})
  	  \item[3.] \gls{Projektbetreuer} füllt die Eingabemaske aus
  	  \item[4.] \gls{Projektbetreuer} speichert die neuen Daten über einen klick
  	  auf \enquote{Speichern} ab
  	\end{enumerate}
  	\item[Erweiterungen:] -keine-
  	\item[Alternativen:] -keine
  	\item[Benötigte FA:] \ref{FAbetreuerChangeProjekt}
  \end{description}
  ...
  \begin{description}
  	...
  	Webseite im Betreuerbereich. Eine finale Einteilung wurde vom Administrator
  	veröffentlicht
  	...
  	\item[Beschreibung:]
  	\begin{enumerate}
  		\item[1.] Der \gls{Projektbetreuer} klickt auf den Namen des Projekts, zu
  		dem er die Ergebnisse erfahren möchte in der Seitenleise in der GUI 
		\item[2.] Der \gls{Projektbetreuer} sieht, welche \glspl{Studierender} in
		welchen \glspl{Team} diesem \gls{Projekt} zugeordnet wurden. Zusätzlich
		erhält er die Information, ob \glspl{Studierender} sich im CMS schon zum
		\gls{PSE} und \gls{TSE} angemeldet haben, die Projektbewertung dieses Projekts
		der Studierenden  und ob noch \glspl{Teilleistung} offen sind
		(\autoref{GUIprojLeiterTeam})
  	\end{enumerate}
  	\item[Erweiterungen:] -keine-
  ...
  \item \label{UCbetreuerLeaveProjekt}
	\begin{description}
  		...
  		\item[Vorbedingung:] Der Benutzer ist \gls{Projektbetreuer} eines
  		\glspl{Projekt}s, in dem noch mindestens ein anderer \gls{Projektbetreuer} ist.
  		\item[Nachbedingung:]	\begin{description}[leftmargin=6em] \item[Erfolg]  Der
  		Benutzer betreut das \gls{Projekt} nicht länger
  		\item[Fehlschlag] Der Benutzer ist immer noch \gls{Projektbetreuer} des
  		\glspl{Projekt}s \end{description}
  		\item[Beschreibung:]
  	\begin{enumerate}
  	  \item[1.] \gls{Projektbetreuer} befindet sich im Beteruerbereich
  	  \item[2.] \gls{Projektbetreuer} wählt das \gls{Projekt} in der Seitenleiste
  	  der GUI aus, welches er verlassen möchte.
  	  \item[3.] \gls{Projektbetreuer} klickt auf den \enquote{Verlassen}"=Knopf
  	  \item[4.] \gls{Projektbetreuer} bestätigt Warnung mit \enquote{Ok}
  	\end{enumerate}
  	\item[Erweiterungen:] -keine-
  	\item[Alternativen:] -keine-
  ...
  \item \label{UCbetreuerNoteneintragung}
    \begin{description}
  	...
  	\item[Vorbedingung:] Betreuer ist Projektbetreuer für ein Team. Die Noten
  	stehen fest
  	...
  	\item[Beschreibung:]
  	\begin{enumerate} 
  	  \item [1.] \gls{Projektbetreuer} klickt auf den Namen des Teams, dessen
  	  Teammitglieder (Studierende) er benoten möchte, in der Seitenleiste der
  	  GUI
  	  \item[1.] \gls{Projektbetreuer} befindet sich nun in der Beterueransicht 
  	  (\autoref{GUIprojLeiterTeam})
  	  \item[2.] \gls{Projektbetreuer} trägt neue Noten für \glspl{Studierender} ein oder
...
\begin{enumerate}
  ...
  \item Nutzer dürfen nur auf ihre eigenen Daten oder über die in den
  funktionalen Anforderungen beschriebenen Schnittstellen auf Daten anderer
  Nutzer zugreifen
\subsubsection{GUI}
{\centering
...
\fbox{\includegraphics[width=\textwidth,
keepaspectratio=true]{gui/adminexportimport.png}}
\captionof{figure}{Ansicht zum Export/Import} \label{GUIexportimport}
\fbox{\includegraphics[width=\textwidth,
keepaspectratio=true]{gui/adminstudentsedit.png}}
\captionof{figure}{Ansicht zur manuellen Verwaltung von Studierenden}
\label{GUIstudentEdit}
\medskip
\fbox{\includegraphics[width=\textwidth,
