...
\subsection{Musskriterien}
 \begin{enumerate}[label=\swtLabel{M}]
   ...
   \item Manuelles Nachjustieren der Einteilung durch den \gls{Admin} möglich
...
\begin{enumerate}[label=\swtLabel{FA}]
  ...
  \begin{itemize}
    ...
    \item Auswahl bestandener Teilleistungen und der \gls{SPO} (vgl. \ref{SDatenAnfang} bis \ref{SDatenEnde})
  \end{itemize}
...
\begin{enumerate}[label=\swtLabel{FA}, resume]
  ...
  \item Konfiguration der Einteilungsparameter \label{FAadminparameter} %TODO Verweis auf GUI? oder Textuell?
...
\begin{enumerate}[label=\swtLabel{FA}, resume]
  ...
  \item Import von \enquote{.csv}"=Dateien mit Informationen über bestandene Leistungen der \glspl{Studierender}n aus dem Campus Management System  \label{FAimport2}
...
\begin{enumerate}
	\item Name
	\item Pflichteilleistungen
	\item Wählbare Leistungen
\end{enumerate}
...
%   
  ...
  \item[Ziel:] \gls{Studierender} kann sehen welchem Projekt er zugeteilt wurde
  	...
  	Falls der \gls{Admin} die Einteilungsergebnisse bereits veröffentlicht hat, kann er
  	sehen, welchem \gls{Projekt} er zugeteilt wurde. Sonst sieht er, dass die Einteilung
  	noch im Gange ist
  	\item[Nachbedingung(Fehlschlag):] Der Studierende ist nicht in der
