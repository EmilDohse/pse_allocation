...
public class CriterionPreferredTeamSize implements GurobiCriterion {
    ...
    public void useCriteria(Configuration configuration,
        ...
        for (int j = 0; j < configuration.getTeams().size(); j++) {
            ...
            isPrefSize = allocator.getModel().addVar(0, 1, 0, GRB.BINARY,
                    GurobiAllocator.NULL);
            auxiliaryVariable = allocator.getModel().addVar(0, 1, 0, GRB.BINARY,
                    GurobiAllocator.NULL);
            ...
            leftSideSecondConstraint.multAdd(-(maxSize + 0.1),
            ...
            allocator.getModel().addConstr(leftSideFirstConstraint,
                    GRB.LESS_EQUAL, varianceToPrefSize, GurobiAllocator.NULL);
            allocator.getModel().addConstr(varianceToPrefSize, GRB.LESS_EQUAL,
                    rightSideFirstConstraint, GurobiAllocator.NULL);

            allocator.getModel().addConstr(leftSideSecondConstraint,
                    GRB.LESS_EQUAL, varianceToPrefSize, GurobiAllocator.NULL);
            allocator.getModel().addConstr(varianceToPrefSize, GRB.LESS_EQUAL,
                    rightSideSecondConstraint, GurobiAllocator.NULL);
