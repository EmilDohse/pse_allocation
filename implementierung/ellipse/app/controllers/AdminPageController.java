...
public class AdminPageController extends Controller {
    ...
    public Result projectPage() {
        ...
        if (GeneralData.loadInstance().getCurrentSemester() == null) {
            ...
            flash(ERROR,
                    error
                            + ctx().messages().at(
                                    ctx().messages().at(NO_SEMESTER)));
            content = views.html.adminProjects.render(new ArrayList<>());
        } else {
            content = views.html.adminProjects.render(GeneralData
                    .loadInstance().getCurrentSemester().getProjects());
        }
    ...
    public Result allocationPage() {
        ...
        play.twirl.api.Html content = views.html.adminAllocation.render(
                AllocationQueue.getInstance(), criteria);
    ...
    public Result exportImportPage() {
        play.twirl.api.Html content = views.html.adminExportImport.render(
                SPO.getSPOs(), GeneralData.loadInstance().getCurrentSemester()
                        .getAllocations());
        Menu menu = new AdminMenu(ctx(), ctx().request().path());
        return ok(views.html.admin.render(menu, content));
    }
    ...
    public Result studentEditPage() {
        ...
        if (GeneralData.loadInstance().getCurrentSemester() == null) {
            ...
            content = views.html.adminStudentEdit.render(GeneralData
                    .loadInstance().getCurrentSemester().getSpos());
        }
        Menu menu = new AdminMenu(ctx(), ctx().request().path());
        return ok(views.html.admin.render(menu, content));
    }
     ...
     * @return Die Seite, die als Antwort verschickt wird.
     */
    public Result projectEditPage(int projId) {
        int id = projId;
    ...
    public Result notAllowedInCurrentState(String url) {
        ...
        case BEFORE_REGISTRATION_PHASE:
            flash("info",
                    ctx().messages().at(
                            "admin.beforeRegistration.actionNotAllowed"));
            break;
        ...
        case AFTER_REGISTRATION_PHASE:
            flash("info",
                    ctx().messages().at(
                            "admin.afterRegistration.actionNotAllowed"));
            break;
        default:
